...
a small, fast-converging reference (2x2x1 mesh, nk=4, no symmetry reduction --
distributed v1 requires use_symmetry=False).
...
def _build_system():
    ...
    return setup_uspp(
        SI_CELL, SI_POS_DISP, [0, 0], [paw], ecut=15 * RY, kmesh=(2, 2, 1), ecutrho=60 * RY
    )
...
def _run(hubbard: bool):
    ...
    man = [HubbardManifold(species=0, l=1, u=3.0, j=0.0)] if hubbard else None
    ref = scf_uspp(_build_system(), PBE(), hubbard=man, **_SCF_KWARGS)
