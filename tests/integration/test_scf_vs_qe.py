...
@pytest.mark.parametrize("name", ["si_lda_ci", "al_pbe_ci"])
def test_lobpcg_matches_davidson(name):
    """Block LOBPCG is a drop-in for Davidson on the NC batched path. Both solve
    the same fixed H each SCF iteration to the same diagonalizer tolerance, so
    the converged free energy must agree. Covers an insulator (Si) and a smeared
    metal (Al)."""
    torch.set_num_threads(4)
    cfg = CASES[name]
    pseudos = cfg["pseudo"] if isinstance(cfg["pseudo"], tuple) else (cfg["pseudo"],)
    ...
    e_dav, e_lob = energy("davidson"), energy("lobpcg")
    assert abs(e_dav - e_lob) / cfg["nat"] < 1e-6, (
        f"{name}: davidson {e_dav:.9f} vs lobpcg {e_lob:.9f}")
