...
class SpinorHamiltonian:
    ...
    def __init__(self, bk: BatchedK, shape, v_r, b_vec_r, p):
        self.bk = bk
        self.shape = shape
    ...
    def apply(self, c: torch.Tensor) -> torch.Tensor:
        ...
        if self.p.shape[1]:
            ...
            out_u = out_u + torch.einsum("kbp,pq,kqg->kbg", bu, dij, self.p)
            out_d = out_d + torch.einsum("kbp,pq,kqg->kbg", bd, dij, self.p)
...
def scf_noncollinear(
    ...
    if xc.needs_gradient:
        raise NotImplementedError("noncollinear GGA needs locally-collinear σ's — LSDA only")
    ...
    for it in range(1, max_iter + 1):
        ...
        h = SpinorHamiltonian(bk, grid.shape, v_r, b_xc, projs_b)
        ...
        e_xc = xc.energy(rho_out, m_out, vol)
        e_loc = local_energy(rho_g_out, vloc_g, vol)
        bu = becp_b(projs_b, coeffs[..., :m_pw])
        bd = becp_b(projs_b, coeffs[..., m_pw:])
        dij = _stack_dij(system)
        e_nl = nonlocal_energy([bu[ik] for ik in range(nk)], dij, occ, system.kweights) \
            + nonlocal_energy([bd[ik] for ik in range(nk)], dij, occ, system.kweights)
