...
class CoarseDraft:
    """Geometry for the certified coarse-box local apply (built once per SCF).

    While the adaptive diagonalization tolerance is loose, the local-potential
    term can run its FFT pair on a smaller box: exactly the ``smooth=`` dual
    grid mechanism the USPP path uses, but for NC — with the potential
    TRUNCATED (not filtered) to the coarse box, so the induced H-apply error
    is bounded by the ℓ1 norm of the dropped V_eff Fourier tail. The draft is
    used only when that certified bound is ≤ 0.1·tol_eff — an error slaved to
    a tolerance the SCF already tolerates by contract — and the final
    iterations plus the convergence gate always run the exact operator, so
    the fixed point (and implicit-diff gradients evaluated there) are
    unchanged.

    Only the strictly conjugate-symmetric Miller set |m_i| ≤ (n_c,i − 1)//2
    is carried (Nyquist planes are counted as tail), keeping the truncated
    potential exactly real and the draft operator exactly Hermitian.
    """

    shape: tuple[int, int, int]
    flat_idx: torch.Tensor  # (nk, npw_max) sphere → coarse box, 0 in padding
    src: torch.Tensor  # (n_kept,) kept Millers, flattened into the FULL box
    dst: torch.Tensor  # (n_kept,) the same Millers, flattened into the coarse box


def build_coarse_draft(
    bk: BatchedK, shape: tuple[int, int, int], frac: float = 0.75
) -> CoarseDraft | None:
    """Coarse-box geometry for `coarse_draft_veff`, or None when not worthwhile.

    The coarse linear sizes are `good_fft_size(max(2·mmax+1, frac·n))` per
    axis: never smaller than the wavefunction-sphere extent (every sphere
    Miller must map to a unique coarse slot for the scatter/gather to be
    exact) and `frac` of the dense grid otherwise (dense is ~2× the sphere
    diameter, so 0.75 keeps the dropped tail small enough to certify against
    early-iteration tolerances while cutting FFT volume ~2.4×). Returns None
    unless the coarse volume is ≤ 70% of the dense volume — below that the
    per-apply saving can't pay for the per-iteration truncation FFT.
    """
    from gradwave.grids import good_fft_size

    n1, n2, n3 = shape
    # signed Millers of every sphere slot, recovered from the dense flat index
    # (padded slots carry flat_idx 0 → Miller 0; they stay masked downstream)
    i1 = bk.flat_idx // (n2 * n3)
    i2 = (bk.flat_idx // n3) % n2
    i3 = bk.flat_idx % n3
    ms = []
    for i, n in ((i1, n1), (i2, n2), (i3, n3)):
        ms.append(torch.where(i > n // 2, i - n, i))
    mmax = [int(m.abs().max()) for m in ms]
    sc = [
        min(good_fft_size(max(2 * mx + 1, int(frac * n))), n)
        for mx, n in zip(mmax, (n1, n2, n3), strict=True)
    ]
    shape_c = (sc[0], sc[1], sc[2])
    if shape_c[0] * shape_c[1] * shape_c[2] > 0.7 * n1 * n2 * n3:
        return None
    c1, c2, c3 = shape_c
    flat_idx_c = ((ms[0] % c1) * c2 + ms[1] % c2) * c3 + ms[2] % c3
    # the strictly symmetric kept set: |m_i| ≤ (n_c,i − 1)//2 on every axis
    axes = [torch.arange(-((c - 1) // 2), (c - 1) // 2 + 1) for c in (c1, c2, c3)]
    m1, m2, m3 = torch.meshgrid(*axes, indexing="ij")
    src = (((m1 % n1) * n2 + m2 % n2) * n3 + m3 % n3).reshape(-1)
    dst = (((m1 % c1) * c2 + m2 % c2) * c3 + m3 % c3).reshape(-1)
    dev = bk.flat_idx.device
    return CoarseDraft(shape_c, flat_idx_c, src.to(dev), dst.to(dev))


def coarse_draft_veff(
    v_eff_r: torch.Tensor, cd: CoarseDraft
) -> tuple[tuple[tuple[int, int, int], torch.Tensor, torch.Tensor], float]:
    """Truncate V_eff onto the coarse box; return (smooth-tuple, error bound).

    The bound is the ℓ1 norm of the dropped Fourier tail [same energy units
    as V_eff]: a convolution-operator sup-norm overbound on ‖H_draft − H‖,
    covering both the truncation and the coarse box's aliasing of the
    retained differences. One dense FFT + one coarse iFFT per call —
    negligible next to the dozens of Davidson applies it accelerates.
    """
    n = v_eff_r.numel()
    vg = torch.fft.fftn(v_eff_r) / n
    kept = vg.reshape(-1)[cd.src]
    eps = float(vg.abs().sum() - kept.abs().sum())
    c1, c2, c3 = cd.shape
    box = torch.zeros(c1 * c2 * c3, dtype=vg.dtype, device=vg.device)
    box[cd.dst] = kept
    v_c = torch.fft.ifftn(box.reshape(c1, c2, c3)).real * (c1 * c2 * c3)
    return (cd.shape, cd.flat_idx, v_c), eps
