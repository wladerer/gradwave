...
# NOTE: the former `_davidson_memory_env` api → environment bridge lived here
# (exporting the scf.memory knobs as GRADWAVE_* env vars for solvers.davidson /
# core.batch to read back). Retired: the knobs now flow as explicit arguments
# (Input → scf.options.MemoryOptions → scf()/scf_uspp()/scf_noncollinear() →
# the solver), and the GRADWAVE_* env vars remain user-facing overrides read
# once per solve where each knob is resolved (see solvers/davidson.py and
# core/batch.py). Config via os.environ mutation was a measured incident class.
