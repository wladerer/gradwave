...
| ~~postscf/uspp_position.py:383,496~~ | nspin2 | position response (density, becsum): nspin=1 only | **removed, this PR (#147)** — threaded the spin axis through `uspp_position`'s own packing layer, the last single-spin piece (the frozen operators `_ConvergedUSPP.{apply_chi0, k_hxc_grid, hvp_onecenter, k_hub}` already thread nspin=2). `PositionPerturbation.__init__` builds a per-spin δv_eff^σ (`dv_r`) and D-rescreening (`d_dscr`) — the NLCC core term now uses the spin f_xc `fxc_hvp_spin` with the core split half/half over the channels — and `dh_ds_psi` indexes both per spin; `bare_map_derivative` loops over spin (the 2.0 δρ prefactor is the ψ*δψ conjugate pair, NOT the spin degeneracy, which lives in `f_win`); `_self_consistent_response` packs the composite (δρ^σ, δbec^σ) + per-channel δn vector through the general `newton._pack/_unpack` (same layout as `newton_polish`/`uspp_implicit`). `bare_position_derivative`/`position_density_response` return per-spin lists for nspin=2, unchanged for nspin=1. Insulator gate stays (metals unchanged). Oracle: `test_nspin2_nonmagnetic_limit_matches_nspin1` (nonmagnetic PAW Si folded into an exact nspin=2 fixed point — scf_uspp has no fixed-spin-moment mode, so a real fixed-occupation nspin=2 SCF cannot produce a valid input; the spin-summed bare/self-consistent δρ and per-spin δbec reproduce nspin=1 to ~1e-13, an exact algebraic identity a factor-2 fold error would miss) |
| ~~postscf/uspp_position.py:588~~ | nspin2 | `hessian_column`: nspin=1 only | **removed, this PR (#147)** — the force-energy graph is now spin-resolved exactly as `paw_forces.forces_uspp` (per-spin coeffs/occ/eps/becsum/ddd state leaves, SpinXC + `spin_sigma_triple` on the total density, per-spin nonlocal/quadratic/ddd/E_U terms), and the δstate pairing that closes the mixed second derivative runs over both channels; the per-spin state response comes from the same `_self_consistent_response`/`_total_orbital_response`. The Hessian column is a single (na, 3) — d²E_total is spin-summed. Oracle: same `test_nspin2_nonmagnetic_limit_matches_nspin1` (nspin=2 column == nspin=1 to ~1e-13). |
...
| file:line | axis | operation blocked |
|---|---|---|
| postscf/uspp_position.py:385,498 | insulator | position response: fixed occupations only (metals need occupation derivatives) |
| postscf/uspp_position.py:590 | insulator | `hessian_column`: insulators only |
