...
projector occupation matrices (becsum), which is the fixed-point variable
of the generalized SCF map. The composite vector is mixed by
Johnson's weighted Broyden method \cite{johnson1988} with equal step
weights for the density and becsum blocks. Ferromagnetic Ni, whose SCF map
has a measured gain of $-3.5$ on the Stoner mode, converges in 16
iterations at the default damping, and the iteration count at matched
convergence criteria agrees with \qe's Broyden mixer. The Pulay mixer required
damping of the becsum block for stability, which slows the same run to 27
iterations. For PAW the one-center energies are
...
layer runs Davidson diagonalization and Broyden-class density mixing under
...
finite-difference floor. Collinear spin systems use the same method,
with per-channel Sternheimer solves and the spin exchange-correlation
kernel, and the shared Fermi level couples the channels through
$\delta\mu$. The spin-degenerate limit reproduces the unpolarized
gradients to $10^{-5}$, triplet O$_2$ matches finite differences to
$2\times10^{-5}$, and ferromagnetic Ni, where both spin channels are
metallic, matches at the $10^{-4}$ level set by the metallic convergence
floor. With a Hubbard manifold active the response vector gains the
occupation-matrix block, whose kernel is the Dudarev second derivative
$-(U-J)$, and the +$U$ gradients match finite differences to
$1.2\times10^{-6}$ with the occupation feedback contributing 1.6\% of
the total.
...
The present limitations are explicit. The electric-field solvers cover
norm-conserving insulators, and the second position derivatives go
through finite differences of the analytic forces rather than one
implicit backward. Neither is structural. The most direct next
steps are LO-TO splitting from the computed Born charges and
magnetocrystalline anisotropy torques $dE/d\hat{\mathbf{m}}$, for which
...
1758 (1999).
\bibitem{johnson1988} D. D. Johnson, Phys. Rev. B \textbf{38}, 12807
(1988).
