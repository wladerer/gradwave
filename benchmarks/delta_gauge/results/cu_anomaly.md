# Cu anomaly: a defective PseudoDojo standard UPF
...
The pseudopotential disagrees with all-electron.
...
**Δ = 1.33 meV/atom**, a normal value. So the fault is specific to the PseudoDojo
`Cu.upf` from the `nc-sr-04_pbe_standard_upf` tarball.

## 3. The same file ships in v0.4 and v0.5, byte-identical

The `Cu.upf` in the v0.4 and v0.5 standard-UPF tarballs have the same md5
(`5fe87f3b4a9befbfed2d60ad3362dad9`), so the discrepancy persists across
PseudoDojo releases rather than being a stale-conversion artifact fixed
downstream.
...
inconsistent with the psp8. The two-axis validation works as intended here (see
