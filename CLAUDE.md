...
| Cap SCF peak memory for a big slab (dense-box chunking + k-streaming + Davidson subspace) | `scf.memory` block on the Input: `k_chunk`, `max_dim_factor`, `subspace_budget_gb`, `subspace_storage`, `dense_budget_gb` (`inputs.models.MemoryParams`; bridged in `api.scf.run_scf` — `k_chunk` kwarg + the `GRADWAVE_*` env via `api._common._davidson_memory_env`). The **binding transient is the dense-grid FFT box** (H-apply/density over all bands), auto band-chunked on CPU when it would exceed ~½ RAM (`scf.loop._auto_cpu_dense_budget`, bit-exact); `dense_budget_gb`/`GRADWAVE_CPU_DENSE_BUDGET` force it, `GRADWAVE_CPU_DENSE_AUTO=off` disables. `k_chunk` is the many-k lever, not the few-k one | set the `GRADWAVE_K_CHUNK`/`GRADWAVE_MAX_DIM_FACTOR`/`GRADWAVE_CPU_DENSE_BUDGET`/… env vars by hand |
