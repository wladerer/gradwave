...
| Cap SCF peak memory for a big slab (dense-box chunking + k-streaming + Davidson subspace) | `scf.memory` block on the Input: `k_chunk`, `max_dim_factor`, `subspace_budget_gb`, `subspace_storage`, `dense_budget_gb` (`inputs.models.MemoryParams`; threaded as arguments via `scf.options.MemoryOptions` in `api.scf.run_scf` — the `GRADWAVE_*` env vars remain user-facing overrides layered over the passed values per solve). The **binding transient is the dense-grid FFT box** (H-apply/density over all bands), auto band-chunked on CPU when it would exceed ~½ RAM (`scf.loop._auto_cpu_dense_budget`, bit-exact); `dense_budget_gb`/`GRADWAVE_CPU_DENSE_BUDGET` force it, `GRADWAVE_CPU_DENSE_AUTO=off` disables. `k_chunk` is the many-k lever, not the few-k one | set the `GRADWAVE_K_CHUNK`/`GRADWAVE_MAX_DIM_FACTOR`/`GRADWAVE_CPU_DENSE_BUDGET`/… env vars by hand |
...
| Trim a slab's wasted vacuum before the SCF (ESM box-independence → cut Nz + npw). **Controllable approximation, NOT exact**: box-independence holds only where density ≈ 0 at the edge; trimming into the diffuse SAD tail shifts the energy (Al: ~tens meV/atom at ~2× npw cut, more beyond). Default is conservative (often a near no-op for light metals); validate ΔE per material and set `vacuum_tol` for the size/accuracy tradeoff | api/YAML path: `slab.vacuum_autosize: true` (auto in `build_system` via `api._slab.resolve_slab_box`); ASE calculator path: `api.trim_slab_vacuum(atoms, pseudos, ecut, boundary="open_z")` returns a trimmed Atoms | assume it is free/exact; trim under `boundary="periodic"` (not box-independent) |
