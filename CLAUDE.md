...
| Trim a slab's wasted vacuum before the SCF (ESM makes the open axis box-independent → cut Nz + npw, exact to the tail) | api/YAML path: `slab.vacuum_autosize: true` (auto in `build_system` via `api._slab.resolve_slab_box`); ASE calculator path: `api.trim_slab_vacuum(atoms, pseudos, ecut, boundary="open_z")` returns a trimmed Atoms | widen/shrink vacuum by hand, or trim under `boundary="periodic"` (not box-independent) |
