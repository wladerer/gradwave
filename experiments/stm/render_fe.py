...
for ax, dat, ttl, cm in [
    (axes[0], tile(up), "spin ↑ LDOS", "afmhot"),
...
fig.suptitle(
    "Fe(100) spin-polarized STM (SP-STM) @ E$_F$, tip 2.5 Å  ·  gradwave postscf.stm  ·  M=8.0 µB",
    fontsize=11,
)
plt.tight_layout()
plt.savefig("experiments/stm/out/fe_sp_stm.png", bbox_inches="tight")
print(
    f"rendered; asym range [{asym.min():+.3f},{asym.max():+.3f}], "
